            ...
            if b.act == 'work':
                if len(b.openSetsCP)>1:
                    if b.currentCP <len(b.openSetsCP):
                        currentCP = font.render("Looking for CP #"+str(b.currentCP),True, BLACK)  
                    else:  
                        currentCP = font.render("Looking for Goal",True, RED)                                                          
                    screen.blit(currentCP,(padding, HEIGHT-35))
            elif b.act == 'fail':
                action = font2.render('No Path Found', True, RED)
            elif b.act == "generating":
                action = font2.render('Generating Map...', True, GOLD)
            if b.act == 'choose':
                choose = font2.render('Choose:',True, D_AQUA)     
                screen.blit(choose, (WIDTH-95, bigtextY+30)) 
